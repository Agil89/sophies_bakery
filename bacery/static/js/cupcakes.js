...
document.onclick = function (e) {
    if (e.target.classList.contains('checked-inputs')) {
        var cupcake_features= new Array()
        document.querySelectorAll('.checked-inputs').forEach(function (e) {
                if (e.checked == true) {
                    var cupcake_feature = e.closest('.d-flex').querySelector('.choose-types').innerText
                    cupcake_features.push(cupcake_feature)
                    
                }
        })
        all_data['cupcake_features'] = cupcake_features
...
function loadAllData(data) {
    ...
    $.ajax({
        url: 'http://104.154.157.225/api/v1.0/products/cupcakes/',
        method: "GET",
        data: data,
        success: function (response) {
            ...
            for (cupcake of response.filtered_cupcakes) {
                document.querySelector('.removed-data').innerHTML +=`
                <div class="col-4 parent-of-card">
                        <div class="card">
                            <img src="${cupcake.main_image}" class="card-img-top" alt="...">
                            <div class="card-body">
                                <h5 class="card-title">${cupcake.name}</h5>
                                <p class="card-text">${cupcake.short_description}</p>
                                <a href="#" class="btn btn-primary w-100">Открыть</a>
                            </div>
                        </div>
                    </div>`
